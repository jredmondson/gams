...
bool SimControl::generate_entities(const double& t) {
    ...
    for (auto ent : ents_)
    {
        cout << " AFTER GENERATING ENTITIES IN GEN_ENTS(T) ID: " << ent->id().id() << endl;
    }
...
bool SimControl::generate_entity(const int &ent_desc_id,
...
#endif
    cout << "GENERATING ENTITY INSIDE SIMCONTROL::GENERATE_ENTITY()" << endl;
    ...
    cout << "ENTS LIST SIZE BEFORE LEAVING GEN ENT FUNCTION " << ents_.size() << endl;
    
    for (auto ent : ents_)
    {
         cout << "ENTITY INSIDE LIST ID IS: " << ent->id().id() << endl;
    }
...
void SimControl::run_remove_inactive() {
    cout << "LOOKING FOR ENTS TO REMOVE " << endl;
    auto it = ents_.begin();
    while (it != ents_.end()) {
        if (!(*it)->active()) {
            int id = (*it)->id().id();
            cout << " LOOKING AT ENTITY ID : " << id << endl;
            (*it)->close(t());
            it = ents_.erase(it);
            contacts_mutex_.lock();
            contacts_->erase(id);
...
bool SimControl::run_single_step(const int& loop_number) {
    ...
    for (auto ent : ents_)
    {
         cout << "BEGINNING OF RUN_SINGLE_STEP() ENT ID : " << ent->id().id() << endl;
    }
    ...
    for (auto ent : ents_)
    {
         cout << "END OF RUN_SINGLE_STEP() ENT ID : " << ent->id().id() << endl;
    }
    
    cout << "SIZE OF ENT LIST BEFORE LEAVING : " << ents_.size() << endl;
    
    if (exit_loop)
    {
       cout << " EXIT LOOP = TRUE " << endl;
    }
...
bool SimControl::start() {
    ...
    auto gen_ent_cb = [&] (auto &msg) {
        ...
        cout << " WE IN THE CALL BACK BABY " << endl;
    };
...
bool SimControl::end_condition_reached() {
    ...
    if (end_conditions_.count(EndConditionFlags::TIME) && t() > mp_->tend() - dt_ / 2.0) {
        ...
        cout << "END CONDITION REACHED: MAX TIME" << endl;
        return true;
    }
