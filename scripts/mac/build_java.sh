#!/bin/bash
# Build the required libraries for GAMS in Java
...
if [ $# -gt 0 ]; then
  if [ "$1" = "lion" ]; then
    ...
    echo "  Build the Java libraries for GAMS"
    ...
    echo "JAVA_HOME           - location of JDK, will be auto-detected if missing"
    exit
fi

$GAMS_ROOT/scripts/mac/build_base.sh vrep java $1
