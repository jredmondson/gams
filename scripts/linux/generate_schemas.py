...
def get_random_hex_id():
    ...
    Return:
        64 bit hex ID used in Capnproto message formats, as string form with @0x prepended.

    """
    return "@0x" + binascii.b2a_hex(os.urandom(8))
...
def generate_schema(message_type, pkg_paths, directory_path=""):
    ...
    variables = get_schema_variables_from_msgspec(msgspec) # TODO
    imports = get_schema_imports_from_msgspec(msgspec) # TODO
...
def get_schema_imports_from_msgspec(msgspec):
    ...
    for t in msgspec_set:
        ...
        if t not in msgs.BUILTIN_TYPES:
            output_string = output_string + "#include " + "\"" + t + ".h\"\n"
            ...
            output_string = output_string + "#include \"String.h\"\n"
...
def handle_static_array(t, name):
    ...
    type_statement = "std::vector<%s> " % atomic_type
...
def handle_dynamic_array(t, name):
    ...
    type_statement = "std::vector<%s> " % base_type
...
def get_schema_variables_from_msgspec(msgspec):
    ...
    for count, (name, type_decl) in enumerate(declarations.iteritems()):
        #output_string = output_string + type_decl + " " + name + ";\n    "
        output_string = name + " @" + str(count) + ": " + type_decl + ";\n"
